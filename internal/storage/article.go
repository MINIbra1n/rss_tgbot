...
func (s *ArticlePostgresStorage) AllNotPostedNotTime(ctx context.Context) ([]model.Article, error) {
	conn, err := s.db.Connx(ctx)
	if err != nil {
		return nil, err
	}
	...
	var articlesAll []dbArticleWithPriority

	if err := conn.SelectContext(ctx, &articlesAll, `SELECT a.id AS a_id,
 				a.title AS a_title,
 				a.link AS a_link
 				FROM articles a
				WHERE a.posted_at IS NULL ;`); err != nil {
		return nil, err
	}
	// log.Printf("------%v------", articlesAll)
	return lo.Map(articlesAll, func(article dbArticleWithPriority, _ int) model.Article {
