...
type ArticleProvider interface {
	...
	AllNotPostedNotTime(ctx context.Context) ([]model.Article, error)
}
...
func (n *Notifier) Start(ctx context.Context) error {
	...
	for {
		...
		case <-ticker.C:
			...
			// if err := n.SelectAndSendAllArticles(ctx); err != nil {
			// 	return err
			// }
		case <-ctx.Done():
			return ctx.Err()
...
func (n *Notifier) SelectAllArticleAndSendOne(ctx context.Context) error { // Не All )))
	top, err := n.articles.AllNotPostedNotTime(ctx)
	// log.Printf("********%v**********", top)
	if err != nil {
		return err
	}
	i := top[0]
	if err := n.sendArticle(i, ""); err != nil {
		log.Printf("[ERROr] %v", err)
		return err
	}
	n.articles.MarkAsPosted(ctx, i)
	// var wg sync.WaitGroup
	// for _, i := range top {
	// wg.Add(1)

	// go func(ctx context.Context, i model.Article) {
	// }(ctx, i)
